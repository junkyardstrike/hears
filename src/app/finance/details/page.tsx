...
function FinanceDetailsContent() {
  ...
  const { items, groupedItems, totalTakeHome, baseSalaryTotal } = useMemo(() => {
    ...
    const now = new Date();
    const monthsToCalculate = (mode === 'year' || mode === 'gross') ? Array.from({ length: 12 }, (_, i) => i + 1) : [monthParam];
      ...
      cases.forEach(c => {
        ...
        if (isStockType) {
          ...
          if (recognitionMonth === mStr) {
            ...
            const item = { id: `${c.id}-shot-${m}`, caseName: c.name, client: c.contractEntity || '未設定', type: '制作費 (SHOT)', amount, date: `${mStr} 計上`, isStock: false, gross, isManualRecognized: !!c.finance.oneTimeFeeMonth, monthLabel };
          ...
          if (mStr === start || isAfter(dMonth, parseISO(start))) {
            ...
            const item = { id: `${c.id}-stock-${m}`, caseName: c.name, client: c.contractEntity || '未設定', type: '保守還元 (STOCK)', amount, date: `${mStr}分`, isStock: true, gross: fee, monthLabel };
          ...
          if (recognitionMonth === mStr) {
            ...
            const amount = isGross ? gross : takeHome;
            const item = { id: `${c.id}-spot-${m}`, caseName: c.name, client: c.contractEntity || '未設定', type: 'スポット収益', amount, date: `${mStr} 計上`, isStock: false, gross, isManualRecognized: !!c.finance.spotMonth, rate, monthLabel };
            results.push(item); grouped[monthLabel].push(item); totalShot += amount;
          }
        }
      });
...
function BreakdownCard({ item, onUpdateBaseSalary }: { item: any, onUpdateBaseSalary: (m: string, a: number) => void }) {
  return (
    <div className={cn("bg-card p-5 rounded-lg border hover:border-primary/50 transition-all flex items-center gap-6 group shadow-sm", item.isBase ? "bg-secondary/30 border-dashed border-border" : "border-border")}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-1">
          ...
          <Badge className={cn("text-[8px] font-bold border-none px-2 rounded-md", item.isBase ? (item.isOverridden ? "bg-emerald-500/20 text-emerald-500" : "bg-secondary text-muted-foreground") : item.isManualRecognized ? "bg-amber-500/20 text-amber-500" : "bg-secondary text-muted-foreground")}>{item.type} {item.isManualRecognized && "• 手動計上"} {item.rate && ` (${item.rate}%)`} {item.isOverridden && "• 個別調整済み"}</Badge>
        </div>
